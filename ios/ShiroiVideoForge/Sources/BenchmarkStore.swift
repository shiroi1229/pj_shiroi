...
struct BenchmarkRecord: Codable, Identifiable, Sendable {
    ...
    let requestedComputePolicy: String?

    init(metrics: GenerationMetrics) {
        id = UUID(); timestamp = metrics.startedAt
        totalSeconds = metrics.totalSeconds; coreMLSeconds = metrics.inferenceSeconds
        metalEncodeSeconds = metrics.metalEncodeSeconds; saveSeconds = metrics.saveSeconds
        outputMegabytes = metrics.outputMegabytes; keyframes = metrics.keyframes
        diffusionStepsPerKeyframe = metrics.diffusionStepsPerKeyframe
        outputFrames = metrics.outputFrames; fps = metrics.fps; quality = metrics.quality.rawValue
        ...
        thermalBefore = metrics.thermalBefore; thermalAfter = metrics.thermalAfter
        lowPowerModeEnabled = metrics.lowPowerModeEnabled
        requestedComputePolicy = metrics.requestedComputePolicy.rawValue
    }
...
actor BenchmarkStore {
    ...
    private var jsonURL: URL { directory.appending(path: "benchmarks.json") }
    ...
    func exportCSV() throws -> URL {
        ...
        var lines = [
            "timestamp,quality,keyframe_backend,requested_temporal,actual_temporal,memory_class,total_s,inference_s,temporal_hevc_s,save_s,output_mb,keyframes,steps_per_keyframe,output_frames,fps,encode_fps,thermal_before,thermal_after,low_power,requested_compute_policy"
        ]
        let iso = ISO8601DateFormatter()
        for record in records {
            lines.append([
                iso.string(from: record.timestamp), csv(record.quality),
                csv(record.keyframeBackend ?? "Core ML SD 1.5 (legacy)"),
                csv(record.requestedTemporalMode ?? "legacy"), csv(record.actualTemporalPath ?? "legacy"),
                csv(record.memoryClass), format(record.totalSeconds), format(record.inferenceSeconds),
                format(record.metalEncodeSeconds), format(record.saveSeconds), format(record.outputMegabytes),
                String(record.keyframes), String(record.diffusionStepsPerKeyframe), String(record.outputFrames),
                String(record.fps), format(record.encodeFramesPerSecond),
                csv(record.thermalBefore), csv(record.thermalAfter),
                record.lowPowerModeEnabled ? "true" : "false", csv(record.requestedComputePolicy ?? "not-recorded")
            ].joined(separator: ","))
        }
        try ensureDirectory()
    ...
    func clear() throws {
        try ensureDirectory()
        if FileManager.default.fileExists(atPath: jsonURL.path) { try FileManager.default.removeItem(at: jsonURL) }
    }

        ...
        GroupBox {
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 16) {
                    ...
                    if metrics.lowPowerModeEnabled { Label("Low Power", systemImage: "battery.25percent") }
                }
                Text("AI実行設定: \(metrics.requestedComputePolicy.title)（使用率の測定値ではない）")
                HStack(spacing: 12) {
            ...
            Label("Device generation benchmark", systemImage: "gauge.with.dots.needle.67percent").font(.headline)
        }
    }

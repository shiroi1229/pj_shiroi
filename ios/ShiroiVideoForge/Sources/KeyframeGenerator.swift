...
actor KeyframeGenerator: KeyframeBackend {
    ...
    /// The app uses ModelManager. A verified local directory enables the same
    /// production backend to be exercised in a native CI executable.
    ...
    func generate(request: GenerationRequest, capabilities: DeviceCapabilities,
        ...
        let pipeline = try pipelineFor(directory: directory, reduceMemory: profile.reduceMemory,
                                       policy: request.computePolicy)
        let count = profile.keyframes
        var images: [CGImage] = []
        var previous: CGImage?
        for index in 0..<count {
            try Task.checkCancellation()
            // Zero motion must not invoke an image-to-image scheduler with zero steps.
            ...
            let result = try pipeline.generateImages(configuration: config) { step in
                ...
                progress(base + local / Double(count),
                    "Core ML keyframe \(index + 1)/\(count) • step \(step.step + 1)/\(step.stepCount)")
                return !Task.isCancelled
            }
            try Task.checkCancellation()
            guard let image = result.compactMap({ $0 }).first else { throw GenerationError.noImage }
            images.append(image)
            previous = image
        }
        return images
    ...
    /// Rebuild the model configuration when processor policy changes. Never label
    /// a policy change successful while reusing models loaded with another policy.
    private func pipelineFor(directory: URL, reduceMemory: Bool,
                             policy: InferenceComputePolicy) throws -> StableDiffusionPipeline {
        if let pipeline, loadedDirectory == directory,
           loadedReduceMemory == reduceMemory, loadedComputePolicy == policy { return pipeline }
        ...
        let created = try StableDiffusionPipeline(resourcesAt: directory, controlNet: [],
            configuration: configuration, disableSafety: false, reduceMemory: reduceMemory)
        do { try created.loadResources() }
        catch { created.unloadResources(); throw error }
        pipeline = created; loadedDirectory = directory
        loadedReduceMemory = reduceMemory; loadedComputePolicy = policy
        return created
    }
    enum GenerationError: LocalizedError {
        case modelMissing, noImage
        var errorDescription: String? {
            switch self {
            case .modelMissing: return "The on-device Core ML model is not installed yet."
            case .noImage: return "No usable image was returned. The safety checker may have filtered it."
            }
        }
    }
}
